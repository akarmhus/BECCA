...
observeEvent(input$ds_go,{
  source("ds_builder.R")
  desc.stats <- ds.builder(ds.env$rows, ds.env$cols, ds.env$dataset)
  desc.percs <- ds.builder(ds.env$rows, ds.env$cols, ds.env$dataset, perc = TRUE)
  output$stats <- renderTable(desc.stats)
  output$percs <- renderTable(desc.percs)
})
```
